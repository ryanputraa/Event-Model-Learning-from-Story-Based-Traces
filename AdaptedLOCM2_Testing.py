...
class LOCM2:
    ...
    def learn(self, data_list):
        for idx, data in enumerate(data_list):
            ...
            for (obj_id, sort), sequences in object_sort_sequences.items():
                ...
                for group_idx, group_sequences in enumerate(independent_sequence_groups, start=1):
                    ...
                    is_well_formed = self.is_well_formed_matrix(transition_matrix, group_sequences)
                    if is_well_formed:
                    ...
                    # Update FSM statistics
                    self.fsm_stats['total_fsms'] += 1
                    num_states = len(fsm.states)
                    num_transitions = sum(len(transitions) for transitions in fsm.transitions.values())
                    self.fsm_stats['total_states'] += num_states
                    self.fsm_stats['total_transitions'] += num_transitions
                    self.fsm_stats['states_per_fsm'].append(num_states)
                    self.fsm_stats['transitions_per_fsm'].append(num_transitions)
                    if is_well_formed:
                        self.fsm_stats['well_formed_fsms'] += 1
                    else:
                        self.fsm_stats['ill_formed_fsms'] += 1
        ...
        # After processing all plans, print FSM statistics
        self.print_fsm_statistics()
    ...
    def print_fsm_statistics(self):
        total_fsms = self.fsm_stats['total_fsms']
        total_states = self.fsm_stats['total_states']
        total_transitions = self.fsm_stats['total_transitions']
        well_formed_fsms = self.fsm_stats['well_formed_fsms']
        ill_formed_fsms = self.fsm_stats['ill_formed_fsms']
        states_per_fsm = self.fsm_stats['states_per_fsm']
        transitions_per_fsm = self.fsm_stats['transitions_per_fsm']

        if total_fsms > 0:
            avg_states_per_fsm = total_states / total_fsms
            avg_transitions_per_fsm = total_transitions / total_fsms
            min_states = min(states_per_fsm)
            max_states = max(states_per_fsm)
            min_transitions = min(transitions_per_fsm)
            max_transitions = max(transitions_per_fsm)
        else:
            avg_states_per_fsm = avg_transitions_per_fsm = 0
            min_states = max_states = min_transitions = max_transitions = 0

        print("\nFSM Statistics:")
        print(f"Total FSMs generated: {total_fsms}")
        print(f"Total states: {total_states}")
        print(f"Total transitions: {total_transitions}")
        print(f"Average states per FSM: {avg_states_per_fsm:.2f}")
        print(f"Average transitions per FSM: {avg_transitions_per_fsm:.2f}")
        print(f"Min/Max states per FSM: {min_states}/{max_states}")
        print(f"Min/Max transitions per FSM: {min_transitions}/{max_transitions}")
        print(f"Well-formed FSMs: {well_formed_fsms}")
        print(f"Ill-formed FSMs: {ill_formed_fsms}")
...
def main():
    ...
    for json_file in json_files:
        ...
        total_plans_processed += len(data_list)

        print(f"FSM output written to {output_file}")
        print(f"Processed {len(data_list)} plans in {execution_time:.2f} seconds")

    if total_plans_processed > 0:
        average_time_per_plan = total_execution_time / total_plans_processed
        print(f"\nTotal execution time: {total_execution_time:.2f} seconds")
        print(f"Average time per plan: {average_time_per_plan:.4f} seconds")
